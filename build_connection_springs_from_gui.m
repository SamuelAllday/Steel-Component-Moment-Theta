...
% Per-segment stiffness (N/mm) and yield forces (N) for tension rows:
conn.tens_K_share  = [K_bolt_share, K_epb_share, K_cfb_share, K_cwt_share, K_bwt_share];
conn.tens_Fy_share = [Ft_Rd_BOLT*1000, Ft_EPB*1000, Ft_CFB*1000, Ft_wc_Rd*1000, Ft_wb_Rd*1000] / nrow;

% Per-segment stiffness/yield for compression chain:
conn.comp_K  = [K_cwc, K_cws, K_bfc];
conn.comp_Fy = [Fc_cw_Rd*1000, Fs_wc_Rd*1000, Fc_BFC*1000];
...
% For Steel01 Materials
conn.tens_K_share  = tens_K_share;
conn.tens_Fy_share = tens_Fy_share;
conn.comp_K        = comp_K;
conn.comp_Fy       = comp_Fy;
end
