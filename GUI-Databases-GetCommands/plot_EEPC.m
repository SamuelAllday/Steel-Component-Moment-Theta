function [figX]=plot_EEPC (app)

clc;
load Database_Bolts.mat

ColSec     = string(app.menuColumn.Value);
BeamSec    = string(app.menuBeam.Value);
Bolt       = string(app.menuBolt.Value);

if app.TS.Value==0; StiffenerC = "No"; end
...
StiffenerC = app.TS.Value;
Load   = app.menuLoad.Value;
Layout = app.menuLayout.Value;

Indx      = find(BoltDB.Name==Bolt);
d_b       = BoltDB.db(Indx);
...
pi = hb - 2* pt/2;  if Layout == "10-bolt"; pi=0; end
p  = 0;
...
% i=1;
% str{i}=['Beam    : ',char(BeamSec),' (',char(GradeB),')'];i=i+1;
% str{i}=['Column : ',char(ColSec),' (',char(GradeC),')'];i=i+1;
